...
import gc
import aiofiles
from config import TELEGRAM_FILE_LIMIT

async def process_download(message, url, is_audio=False, is_video_trim=False, is_audio_trim=False, 
                          start_time=None, end_time=None, bot=None, upload_to_mega=None, 
                          send_message=None, logger=None, detect_platform=None, PLATFORM_HANDLERS=None):
    """Handles video/audio download and sends it to Telegram or MegaNZ."""
    try:
        ...
        # Import specific handlers as needed - reduces dependencies in the global scope
        from handlers.youtube_handler import extract_audio_ffmpeg
        ...
        for file_path in file_paths:
            ...
            if file_size > TELEGRAM_FILE_LIMIT or file_size > 49 * 1024 * 1024:
                ...
                logger.info(f"File too large for Telegram: {file_size} bytes. Using MegaNZ.")
                mega_link = await upload_to_mega(file_path, filename)

                if mega_link:
                    logger.info(f"Successfully uploaded to MegaNZ: {mega_link}")
                    await send_message(
                        message.chat.id,
                        f"⚠️ **File too large for Telegram.**\n📥 [Download from MegaNZ]({mega_link})"
                    )
                else:
                    logger.warning("MegaNZ upload failed")
                    if download_url:
                        await send_message(
                            message.chat.id,
                ...
                try:
                    ...
                    if "413" in str(send_error):
                        logger.info("Got 413 error, attempting MegaNZ upload as fallback")
                        filename = f"{message.chat.id}_{os.path.basename(file_path)}"
                        mega_link = await upload_to_mega(file_path, filename)
                        ...
                        if mega_link:
                            ...
                            await send_message(message.chat.id, "❌ **File too large and MegaNZ upload failed.**")
                    else:
...
async def process_image_download(message, url, bot=None, upload_to_mega=None, 
                                send_message=None, logger=None, detect_platform=None, PLATFORM_PATTERNS=None):
    """Handles image download and sends it to Telegram or MegaNZ."""
    try:
        await send_message(message.chat.id, "🖼️ Processing Instagram image...")
        logger.info(f"Processing Instagram image URL: {url}")
        
        # Import handler locally to reduce global dependencies
        from handlers.image_handlers import process_instagram_image
        ...
        try:
            ...
            for file_path in file_paths:
                ...
                if file_size > TELEGRAM_FILE_LIMIT:
                    filename = f"{message.chat.id}_{os.path.basename(file_path)}"
                    logger.info(f"Image too large for Telegram: {file_size} bytes. Using MegaNZ.")

                    # Upload to MegaNZ
                    mega_link = await upload_to_mega(file_path, filename)

                    if mega_link:
                        logger.info(f"Successfully uploaded image to MegaNZ: {mega_link}")
                        await send_message(
                            message.chat.id,
